#' Twin K Support Vector Machine for Multi-classification with Pin-ball loss
#'
#' @author Zhang Jiaqi
...
#' @return return pintwinKsvm object
#' @export
pintwinKsvm <- function(X, y,
                        Ck = rep(1, 4), tau = 0.5,
  ...
  for (i in 1:class_num) {
    for (j in i:class_num) {
      ...
      lbA1 <- matrix(-tau*Ck[1], nrow = mB)
      lbA2 <- matrix(-tau*Ck[2], nrow = mC)
      lbA <- rbind(lbA1, lbA2)
      ...
      lbB1 <- matrix(-tau*Ck[3], nrow = mA)
      lbB2 <- matrix(-tau*Ck[4], nrow = mC)
      lbB <- rbind(lbB1, lbB2)
  ...
  pintwinKsvm <- list('X' = X, 'y' = y,
                   ...
                   'Rcpp' = rcpp,
                   'call' = match.call())
  class(pintwinKsvm) <- "pintwinKsvm"
  return(pintwinKsvm)
}

#' Predict Method for Twin K Support Vector Machine with Pin-ball loss
#'
#' @author Zhang Jiaqi
...
predict.pintwinKsvm <- function(object, X, y, ...){
  X <- as.matrix(X)
  y <- as.matrix(y)
  m <- nrow(X)
  km <- nrow(object$X)
...
#' Computes K-fold Cross-Validation Accuracy for Twin K svm with Pin-ball loss
#'
#' @author Zhang Jiaqi
#' @param X a new data frame for predicting.
#' @param y a label data frame corresponding to X.
...
cv.pintwinKsvm <- function(X, y, K = 5, tau = 0.5,
                        C1 = 1, C2 = 1,
                        kernel = c('linear', 'rbf', 'poly'),
                        gamma = 1 / ncol(X), degree = 3, coef0 = 0,
                        reg = 1e-7, kernel_rect = 1,
  ...
  param <- expand.grid(C1, C2, gamma, degree, coef0, eps, tau)
  m <- nrow(X)
  if (shuffle == TRUE) {
  ...
  res <- foreach::foreach(j = 1:nrow(param), .combine = rbind,
    ...
    for (i in 1:K) {
      ...
      test_y <- y[new_idx_k]
      train_y <- y[-new_idx_k]
      twinKsvm_model <- pintwinKsvm(train_X, train_y, tau = param[j, 7],
  ...
  cat("Best Parameters :",
      ...
      "eps =", param[max_idx, 6], "tau = ", param[max_idx, 7],
      "\n",
