#' LSSVM for Multi-classification by Using Ones versus Rest Strategy
#'
#' \code{lssvm_ovr} is an R implementation of support vector machine
#'
#' @author Zhang Jiaqi.
...
#' @return return \code{lssvm_ovr} object.
#' @export

lssvm_ovr <- function(X, y,
                      C = 1.0,
  ...
  for (k in 1:class_num) {
    ...
    x <- solve(H + diag(rep(1/C, m)), matrix(1, nrow = m))
    alphas <- as.matrix(x)
    coef_list[, k] <- y_temp_mat %*% alphas
    if (fit_intercept == TRUE) {
      temp <- y_temp - KernelX %*% coef_list[, k]
      intercept_list[k] <- mean(temp)
    }
  }

  lssvm_ovr_model <- list('X' = X, 'y' = y,
  ...
  if (fit_intercept == TRUE) {
    lssvm_ovr_model$intercept <- intercept_list
  }
  class(lssvm_ovr_model) <- "lssvm_ovr"
  return(lssvm_ovr_model)
}


#' Predict Method for Least Squares Support Vector Machine
#'
#' @author Zhang Jiaqi
#' @param object A fitted object of class inheriting from \code{lssvm_ovr}.
...
predict.lssvm_ovr <- function(object, X, y, ...) {
  X <- as.matrix(X)
  y <- as.matrix(y)
  m <- nrow(X)
...
#' Computes K-fold Cross-Validation Accuracy for lssvm_ovr
#'
#' @author Zhang Jiaqi
#' @param X,y dataset and label.
#' @param C plenty term.
...
cv.lssvm_ovr <- function(X, y , K = 5, C = 1,
                         kernel = c('linear', 'rbf', 'poly'),
                         gamma = 1 / ncol(X), degree = 3, coef0 = 0,
                         rcpp = TRUE, fit_intercept = TRUE,
  ...
  res <- foreach::foreach(j = 1:nrow(param), .combine = rbind,
    ...
    for (i in 1:K) {
      ...
      test_y <- y[new_idx_k]
      train_y <- y[-new_idx_k]
      lssvm_ovr_model <- lssvm_ovr(train_X, train_y,
                                   C = param[j, 1],
                                   kernel = kernel,
                                   gamma = param[j, 2], degree = param[j, 3],
                                   coef0 = param[j, 4],
                                   fit_intercept = fit_intercept)
      pred <- predict(lssvm_ovr_model, test_X, test_y)
